(function () {
	...
	let res = []
	let sound
	const audio1 = new Audio('SOUND/mnok.mp3');
	const audio2 = new Audio('SOUND/mok.mp3');
	const audio3 = new Audio('SOUND/mfail.mp3');
	const audio4 = new Audio('SOUND/mend.m4a');

	window.onload = ()=>{
		a1v=container[0].value=devine[0];
		a2.focus();
	};
	...
	function background() {
		...
		if (maxTry != 0) {
			...
			if (a1v !== "" && a2v !== "" && a3v !== "" && a4v !== "" && a5v !== "") {

				// On determine si la case corespond a son emplacement dans le mot a trouver et on assigne la couleur du bg
				// On sauvegarde avec o pour remettre la valeur dans un input si elle est bonne
				let a = [a1v, a2v, a3v, a4v, a5v]
				let b = [a1b, a2b, a3b, a4b, a5b]
				let c = [a1, a2, a3, a4, a5]
				let o = [o1v, o2v, o3v, o4v, o5v]


				for (let i = 0; i <= 4; i++) {
					if (a[i] == devine[i]) {
						b[i] = c[i].style.background = "green"
						o[i] = a[i]

					} else if (devine.match(a[i])) {
						b[i] = c[i].style.background = "yellow"
						o[i] = null
					} else {
						b[i] = c[i].style.background = "red"
						o[i] = null
					}
	...
	function controlMot() {
		...
		res = []
		res.push(a1v, a2v, a3v, a4v, a5v)
		res = res.join('')
		if (aDeviner.includes(res)) {
			valide = true
		} else {
			valide = false
		}
	...
	function modifHTML() {
		...
		// On retire tout les enfants de la ligne actuelle

		for (let i = ligne[count].childNodes.length; i >= 1; i--) {

			ligne[count].removeChild(ligne[count].firstChild)
	...
	function modifHTML1() {
		...
		//Si le mot est valide (dans le dico)

		if (valide === true) {
			res = []
			res.push(a1v, a2v, a3v, a4v, a5v)
			//Si le resultat est bon ou pas
			if (res.join('') == devine) {
				sound = audio2.play()
			} else { sound = audio1.play() }
			// On remplace les inputs par une case avec la lettre et le bg associé
			setTimeout(function () {
				sound
				ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: ${a1b};">${a1v}</p></td>`)
			}, 400)
			setTimeout(function () {
				ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: ${a2b} ;">${a2v}</p></td>`)
			}, 800)
			setTimeout(function () {
				ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: ${a3b} ;">${a3v}</p></td>`)
			}, 1200)
			setTimeout(function () {
				ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: ${a4b} ;">${a4v}</p></td>`)
			}, 1600)
			setTimeout(function () {
				ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: ${a5b} ;">${a5v}</p></td>`)
			}, 2000)
			setTimeout(function () { modifHTML2() }, 2200)
		} else
		//Si le mot est pas valide(pas dans le dico)
		{
			audio3.play()
			ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: red;">${a1v}</p></td>`)
			ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: red;">${a2v}</p></td>`)
			ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: red;">${a3v}</p></td>`)
			ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: red;">${a4v}</p></td>`)
			ligne[count].insertAdjacentHTML('beforeend', `<td><p style="background: red;">${a5v}</p></td>`)
			modifHTML2()
		}
	}
	...
	function modifHTML2() {
		...
		count++
		//Decrementation des essais
		maxTry--
		//Si il reste des essais on remplace la ligne suivante par une nouvelle ligne d'input
		if (maxTry != 0) {
			for (let i = ligne[count].childNodes.length; i >= 1; i--) {
				// On enleve tout les enfants de la ligne suivante
				ligne[count].removeChild(ligne[count].firstChild)
				//Si la lettre corespond on reécrit sa valeur dans le prochaine input
				//Dans tous les cas on crée une nouvelle ligne d'input
			}
			if (o1v !== null) {
				ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a1" maxlength="1" value="${a1v}"></td>`)
			} else { ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a1" maxlength="1"></td>`) }
			if (o2v !== null) {
				ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a2" maxlength="1" value="${a2v}"></td>`)
			} else { ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a2" maxlength="1"></td>`) }
			if (o3v !== null) {
				ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a3" maxlength="1" value="${a3v}"></td>`)
			} else { ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a3" maxlength="1"></td>`) }
			if (o4v !== null) {
				ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a4" maxlength="1" value="${a4v}"></td>`)
			} else { ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a4" maxlength="1"></td>`) }
			if (o5v !== null) {
				ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a5" maxlength="1" value="${a5v}"></td>`)
			} else { ligne[count].insertAdjacentHTML('beforeend', `<td><input id="a5" maxlength="1"></td>`) }
	...
	function resultat() {
		...
		if (maxTry != 0) {
			...
			if (res.join('') == devine) {
				sound = audio2.play()
				document.querySelector('div').innerHTML = `<p> Gagné le bon mot est ${devine}</p>`
				setTimeout(function () {
					window.location.reload(true);
				}, 6000)
			} else {
				//Si le mot n'est pas bon on met le focus sur le prochain input
				nextInput()
			...
			if (res.join('') == devine) {
				...
				//Si il n'y a plus d'essais et que le mot n'est pas bon	
				audio4.play();
				document.querySelector('div').innerHTML = `<p> Perdu le bon mot est ${devine}</p>`
				setTimeout(function () {
					window.location.reload(true);
				}, 6000)
			}
	...
	// Autofocus sur les inputs
	//Si c'est la touche retour on efface la valeur et on repart sur le precedent
	//Sinon on focus le prochain input

	container[0].addEventListener('keyup', function (event) {
		if (event.keyCode == 8) {
			this.value = ""
	...
	container[1].addEventListener('keyup', function (event) {
		if (event.keyCode == 8) {
	...
	container[2].addEventListener('keyup', function (event) {
		if (event.keyCode == 8) {
			this.parentElement.previousElementSibling.querySelector('input').focus()
	...
	container[3].addEventListener('keyup', function (event) {
		if (event.keyCode == 8) {
			this.parentElement.previousElementSibling.querySelector('input').focus()
	...
	container[4].addEventListener('keyup', function () {
		if (event.keyCode == 8) {
			this.parentElement.previousElementSibling.querySelector('input').focus()
		} 
		else {
	...
	function nextInput() {
		container = document.querySelectorAll("input");
		if (maxTry != 0) {
			...
			if (maxTry == 1) {
				...
				container[4].addEventListener('keyup', function (event) {
					if (event.keyCode == 8) {
						this.parentElement.previousElementSibling.querySelector('input').focus()
